...
export default function WaiterDashboard() {
    ...
    const allWaiterCalls = useQuery(api.waiterCalls.getWaiterCallsByRestaurant,
        restaurantId ? { restaurantId: restaurantId, status: "pending" } : "skip"
    );

    const allReadyOrders = useQuery(api.orders.getOrdersByRestaurant,
        restaurantId ? { restaurantId: restaurantId, status: "ready" } : "skip"
    );

    const allTables = useQuery(api.tables.getTablesByRestaurant,
        restaurantId ? { restaurantId: restaurantId } : "skip"
    );

    const allApprovalOrders = useQuery(api.orders.getOrdersByRestaurant,
        restaurantId ? { restaurantId: restaurantId, status: "needs_approval" } : "skip"
    );
